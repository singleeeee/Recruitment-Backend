...
export class ApplicationController {
  ...
  async getDashboard(@Request() req: { user: AuthenticatedUser }) {
  ...
  @ApiResponse({ status: 200, description: '获取申请列表成功' })
  async findAll(@Query() query: ApplicationQueryDto, @Request() req: { user: AuthenticatedUser }) {
  ...
  @ApiResponse({ status: 200, description: '获取我的申请成功' })
  async getMyApplications(@Request() req: { user: AuthenticatedUser }) {
  ...
  @ApiResponse({ status: 403, description: '没有权限查看该申请' })
  ...
  @ApiResponse({ status: 201, description: '提交申请成功' })
  @ApiResponse({ status: 400, description: '参数验证失败或申请条件不满足' })
  @ApiResponse({ status: 404, description: '招新不存在' })
  @ApiResponse({ status: 409, description: '您已申请过该招新' })
  async create(
  ...
  @ApiResponse({ status: 403, description: '没有权限修改申请状态' })
  @ApiResponse({ status: 404, description: '申请不存在' })
  async updateStatus(
    @Param('id') id: string,
  ...
  @ApiResponse({ status: 403, description: '没有权限删除该申请' })
  @ApiResponse({ status: 404, description: '申请不存在' })
  ...
  async remove(
    ...
    return this.applicationService.remove(id, req.user.id, req.user.role);
  }
